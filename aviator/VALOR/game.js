...
class Game {
    constructor( obj ){ 
        ...
        this.game_create({}); 
        this.get_history(); 
        this.get_bets({ user:$user.uid, sort:'id', dir:'desc', status:7 });
        this.bind(); 
    ...
    bind(){
        ...
        $('#actions_wrapper .actions_field .ranger input').off().on('keyup', function(){
            ...
            var $val = +$self.val(); 
            $val = $val < 0.5 ? 0.5 : ( $val > 100 ? 100 : $val ); 
            $self.val( $val ); 
    ...
    update( obj ){
        ...
        var $change = false; 
        //console.log( $delta );
        if( $delta >= this.timers[ this.status ] ){
            $change = true; 
            this.timer = $timer; 
        }
        switch( this.status ){
                ...
                if( $change ){ 
                    ...
                    $('.make_bet').each(function(){
                        ...
                        if( $id ){
                            ...
                            $self.addClass('danger').removeClass('warning'); 
                            $('span', $self).html(LOCALIZATION.make_bet_generic_cancel).css('display', 'flex');
                            $('h2', $self).hide();
                            $('h3', $self).css('display', 'flex');
                        }
                    }); 
                    ...
                    $('#complete_level').css('display', 'none'); 
                    $('#bets_wrapper .progresser').css('width', '100%'); 
                    ...
                    $freq = $freq - ( $delta / ( this.timers["loading"] / 100 ) );
                    $('#loading_level .progresser').css('width', $freq+"%").attr('data-freq',$freq); 
                    this.bet_generic();
                }
                break;
            case "flight":
                if( $change || ( this.cur_cf >= this.win_cf ) ){ 
                    this.status = "finish"; 
                    $plane.trace = false; 
                    $plane.pos = 5; 
                    ...
                    //if( Math.random()*100 > 70 ){ this.cur_cf = ( this.cur_cf * 1 ) + 0.01; }
                    //this.cur_cf = ( this.cur_cf * 100 / 100 ).toFixed(2); 
                    ...
                    $('#bets_wrapper .info_window [data-rel="bets"] .label').html( $total_wins.toFixed(2) ); 
                    var $players = $('#current_bets_list ul li').length; 
                    var $winners = $('#current_bets_list ul li.active').length ; 
                    var $perc = $winners / ( $players / 100 )
                    $('#bets_wrapper .info_window [data-rel="bets"] .cur').html( $winners ); 
    ...
    bet_complete( $data ){ 
        $.ajax({
            ...
            success: function($r){
                ...
                $game.get_bets({ user:$user.uid, sort:'id', dir:'desc', status:7 });
            }
        });
    }
    bet_generic( $data ){
        if( $users && $users.length ){ 
            ...
            for( var $u of $users ){
                if( ( Math.random() * 100 >= 99.5 ) && $u.name ){ 
                    var $add = true; 
                    ...
                    if( $add ){ 
                        ...
                        this.current_bets.push({ uid:$u.uid, name:$u.name, amount:$amount, cf:$cf, img:$u.img  }); 
                        $('#game_bets .label').html( this.current_bets.length ); 
                        $('#bets_wrapper .info_window [data-rel="bets"] .cur').html( this.current_bets.length );
                        $('#bets_wrapper .info_window [data-rel="bets"] .total').html( this.current_bets.length );
                    }
                }
            }
        }

    ...
    update( obj ){
        ...
        switch( this.status ){
            case "loading": 
                ...
                break;
            case "flight":
                // Коэффициент обновляется только от WebSocket сервера
                // Локальные вычисления отключены
...
var socket = io.connect('https://valor-games.com');    
socket.on('message', ( msg ) => { 
    ...
    if( $obj && $obj.msg && $obj.msg == "Change game state" ){
        ...
        // Всегда используем коэффициент от WebSocket сервера

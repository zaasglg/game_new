...
<body>
  ...
  <script>
    ...
    class HackWebSocketClient {
      ...
      init() {
        // Получаем user_id из URL или запрашиваем у пользователя
      ...
      requestUserId() {
        const userId = prompt('Введите User ID для подключения к игре:', 'cr_demo_' + Date.now());
        if (userId) {
      ...
      setupEventListeners() {
        ...
        this.socket.on('registered', (data) => {
          console.log('👤 Hack bot registered successfully:', data);
          this.showConnectionStatus('Зарегистрирован как хак-бот', true);
        });
        
        this.socket.on('game_connected', (data) => {
          console.log('🎮 Game connected:', data);
          this.gameConnected = true;
          this.showConnectionStatus('Игра подключена', true);
        ...
        this.socket.on('peer_disconnected', (data) => {
          if (data.disconnected_type === 'game') {
            this.gameConnected = false;
            this.showConnectionStatus('Игра отключена', false);
          }
        });
      }
      
      // Отправка прогноза игре
      sendPrediction(prediction) {
        ...
        console.log('📤 Sending prediction:', predictionData);
      ...
      requestAnalysis() {
        ...
        console.log('📤 Requesting analysis:', requestData);
      ...
      performAutoPrediction() {
        if (this.gameConnected) {
          ...
          // Fallback на обычное прогнозирование
          this.generateAndDisplayPrediction();
        }
      }
      
      // Генерация и отображение прогноза
    ...
    // Генерация предсказания с использованием seeded random для большей точности
    function generateRealPrediction(difficulty) {
      ...
      // Используем seed для более детерминированного предсказания
      ...
      if (firstRoll < 20) {
        flameSegment = 0;
        console.log('Flame на первом шаге (20% случай)');
      } else {
        // 80% случай - используем обычную логику
      ...
      const roadSegments = [];
      const totalSegments = Math.min(15, cfs.length + 5); // Больше сегментов для лучшей визуализации
      
      for (let i = 0; i < totalSegments; i++) {
        let multiplier;
        
        // Используем реальные множители из CFS
        if (i < cfs.length) {
          multiplier = cfs[i];
        } else {
          // Если превышаем длину CFS, экстраполируем по формуле роста
      ...
      // Конфиденциальность зависит от сложности и позиции flame
      let confidence;
      if (flameSegment === 0) {
        confidence = Math.floor(95 + rng.nextFloat() * 4); // 95-99% для критических случаев
      ...
      if (gameState.predictionHistory.length > 50) {
        gameState.predictionHistory.shift(); // Ограничиваем историю
      }
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      document.getElementById('analyze-btn').addEventListener('click', function() {
        ...
        // Если WebSocket подключен и игра доступна, запрашиваем анализ
        if (window.HackWS && window.HackWS.isConnected && window.HackWS.gameConnected) {
          console.log('🌐 Requesting analysis via WebSocket...');
          const analysisRequested = window.HackWS.requestAnalysis();
          
          if (analysisRequested) {
            // Ждем ответ через WebSocket (таймаут 5 секунд)
            const timeout = setTimeout(() => {
              console.log('⏰ WebSocket analysis timeout, falling back to local prediction');
      ...
      document.getElementById('analyze-btn').performLocalAnalysis = function() {
        setTimeout(() => {
          ...
          // Отправляем прогноз через WebSocket если подключены
